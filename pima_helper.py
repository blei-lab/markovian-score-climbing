...
# Load data
def load_data(seed=npr.RandomState(1234)):
    fname = 'data/pima-indians-diabetes.csv'
    regressor_data = np.loadtxt(fname, delimiter=',', skiprows=9, usecols=range(8)) 
    regressor_data = np.insert(regressor_data, 0, 1, axis=1)
    outcome_data = np.loadtxt(fname, dtype=str, delimiter=',', usecols=8)
    # Binarize
    outcome_data = (outcome_data == '1').astype('float').reshape((len(outcome_data),1))
