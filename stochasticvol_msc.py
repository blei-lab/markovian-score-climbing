...
def compute_likelihood(params, seed, Np):
    ...
    for t in range(T+1):
        ...
        # Weights
        logW = log_weights_twisting(t, latent_samples[:,t], latent_samples[:,t-1], observations, var_params, mod_params)
        maxLogW = np.max(logW)
...
@notrace_primitive
def generate_samples(params, seed, latent_prev):
    var_params, mod_params = params
    ...
    for t in range(T+1):
        ...
        if t > 0:
            ancestors = resampling(W, seed)
            latent_samples[:N,:t] = latent_samples[ancestors[:N],:t]
        
        # Propagate
        latent_samples[:,t] = sim_variational_approx(t, latent_samples[:,t-1], var_params, mod_params, seed)
        latent_samples[-1,t] = latent_prev[t]
        
        # Ancestor sampling
        if t > 0:
            logW = np.log(W) + log_variational_approx(var_params, mod_params, t, latent_prev[t], latent_samples[:,t-1])
            maxLogW = np.max(logW)
            W = np.exp(logW-maxLogW)
            W /= np.sum(W)
            A = discretesampling(W, seed)
            latent_samples[-1,:t] = latent_samples[A,:t]
...
def objective(params, iter):
    global latent_prev
    var_params, mod_params = params
    latent_prev = generate_samples(params, seed, latent_prev)
...
np.save('results/stochasticvol_N'+str(N)+'_stepsize'+str(step_size)+'_infvec_msc.npy', inf_vec)
np.save('results/stochasticvol_N'+str(N)+'_stepsize'+str(step_size)+'_loglbda_msc.npy', loglbda_vec)
np.save('results/stochasticvol_N'+str(N)+'_stepsize'+str(step_size)+'_logZ_msc.npy', logZ)