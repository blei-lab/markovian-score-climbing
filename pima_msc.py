...
from pima_helper import *


# Initialize
seed_data = npr.RandomState(1234)
seed = npr.RandomState(123)
S = 10
init_scale = 0.1

@notrace_primitive
def generate_samples(params, seed, zprev, verbose):
    mu, log_sigma = params
    ...
    if verbose:
        print('ESS: '+str(1./np.sum(w**2)))
    return w, z, z[:,idx]

def objective(params, iter, verbose=False):
    global zprev
    w, z, zprev = generate_samples(params, seed, zprev, verbose)
...
for i in range(num_indep):
    ...
    zprev = init_mu + np.exp(init_logsigma)*seed.normal(size=nz)
...
np.save('results/pima_S'+str(S)+'_stepsize'+str(step_size)+'_prederror_msc.npy', pred_error)
np.save('results/pima_S'+str(S)+'_stepsize'+str(step_size)+'_params_msc.npy', np.array(opt_params))
np.save('results/pima_S'+str(S)+'_stepsize'+str(step_size)+'_postpredval_msc.npy', np.array(post_pred_val))
