...
def load_data(seed=npr.RandomState(1234),complete=False):
    fname = 'data/ionosphere.data'
    regressor_data = np.loadtxt(fname, delimiter=',', usecols=range(34)) 
    regressor_data = np.insert(regressor_data, 0, 1, axis=1)
    outcome_data = np.loadtxt(fname, dtype=str, delimiter=',', usecols=34)
    # Binarize
    outcome_data = (outcome_data == 'g').astype('float').reshape((len(outcome_data),1))
